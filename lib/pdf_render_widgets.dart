...
import 'package:pdf_render/src/widget/pdf_render_interactive_viewer.dart';
...
class PdfPageView extends StatefulWidget {
  ...
  const PdfPageView({Key? key, this.pdfDocument, required this.pageNumber, this.pageBuilder})
      : super(key: key);
...
class _PdfPageViewState extends State<PdfPageView> {
  ...
  Size _sizeFromConstraints(BoxConstraints constraints, Size pageSize) {
    final ratio =
        min(constraints.maxWidth / pageSize.width, constraints.maxHeight / pageSize.height);
  ...
  Future<bool> _buildTexture(
      {required Size size, bool backgroundFill = true, double? renderingPixelRatio}) async {
    if (_doc == null ||
    ...
    if (_texture == null ||
      ...
      _texture?.dispose();
      _texture =
          await PdfPageImageTexture.create(pdfDocument: _doc!, pageNumber: widget.pageNumber!);
    }
...
class PdfViewerController extends TransformationController {
  ...
  Matrix4? calculatePageFitMatrix({required int pageNumber, double? padding}) =>
      calculatePageMatrix(
          pageNumber: pageNumber, padding: padding, x: 0, y: 0, anchor: PdfViewerAnchor.topLeft);
  ...
  Matrix4? calculatePageMatrix({
    ...
    final left =
        _clipX((rect.left + rect.width * x) * ratio - viewWidth * (anchor.index % 3) / 2, destZoom);
    final top = _clipY(
        (rect.top + rect.height * y) * ratio - viewHeight * (anchor.index ~/ 3) / 2, destZoom);
  ...
  Rect get viewRect => Rect.fromLTWH(
      -value.row0[3], -value.row1[3], _state!._lastViewSize!.width, _state!._lastViewSize!.height);
...
class _PdfViewerState extends State<PdfViewer> with SingleTickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        return PDFRenderInteractiveViewer(
  ...
  void _relayout(Size? viewSize) {
    ...
    if (widget.params?.layoutPages == null) {
      ...
      final rects =
          widget.params!.layoutPages!(contentSize, _pages!.map((p) => p.pageSize).toList());
  ...
  void _relayoutDefault(Size viewSize) {
    if (widget.params?.scrollDirection == Axis.horizontal) {
      final maxHeight =
          _pages!.fold<double>(0.0, (maxHeight, page) => max(maxHeight, page.pageSize.height));
      ...
      final maxWidth =
          _pages!.fold<double>(0.0, (maxWidth, page) => max(maxWidth, page.pageSize.width));
  ...
  Iterable<Widget> iterateLaidOutPages(Size viewSize) sync* {
    if (!_firstControllerAttach && _pages != null) {
      ...
      final r = m.row0[0];
      final exposed =
          Rect.fromLTWH(-m.row0[3], -m.row1[3], viewSize.width, viewSize.height).inflate(_padding);

      for (final page in _pages!) {
        if (page.rect == null) continue;
        final pageRectZoomed = Rect.fromLTRB(
            page.rect!.left * r, page.rect!.top * r, page.rect!.right * r, page.rect!.bottom * r);
        ...
        yield Positioned(
          ...
          child: Container(
            ...
            decoration: widget.params?.pageDecoration ??
                const BoxDecoration(color: Color.fromARGB(255, 250, 250, 250), boxShadow: [
  ...
  void _determinePagesToShow() {
    ...
    final r = m.row0[0];
    final exposed =
        Rect.fromLTWH(-m.row0[3], -m.row1[3], _lastViewSize!.width, _lastViewSize!.height);
    var pagesToUpdate = 0;
    var changeCount = 0;
    _visiblePages.clear();
    for (final page in _pages!) {
  ...
  Future<void> _updateRealSizeOverlay() async {
    ...
    for (final page in _pages!) {
      ...
      if (page.preview?.hasUpdatedTexture == true &&
        ...
        page.realSizeOverlayRect =
            Rect.fromLTWH(offset.dx / r, offset.dy / r, part.width / r, part.height / r);
  ...
  Future<void> _goTo(
      {Matrix4? destination, Duration duration = const Duration(milliseconds: 200)}) async {
    try {
      if (destination == null) return; // do nothing
      _animGoTo?.removeListener(_updateControllerMatrix);
      _animController.reset();
      _animGoTo =
          Matrix4Tween(begin: _controller!.value, end: destination).animate(_animController);
